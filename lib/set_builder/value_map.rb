module SetBuilder
  module ValueMap
    ...
    def self.registered?(name)
      name = name.to_sym
      __registered_value_maps.key?(name)
    end
    ...
    def self.to_s(name, value)
      ...
      name = name.to_sym
      map = __registered_value_maps[name]
      if map
    ...
    def self.for(name)
      name = name.to_sym
      __registered_value_maps[name] || raise(ArgumentError, "A value map has not been registered for #{value}")
    end
    ...
    def self.register(name, map)
      ...
      name = name.to_sym
      __registered_value_maps[name] = map
    end

    def self.as_json(options={})
      __registered_value_maps.dup
    end

    def self.to_json
      __registered_value_maps.to_json
    end
  end
end
