...
SetBuilder.Trait = function(_raw_data) {
  ...
  function findAll(tokenOrTokens) {
    return _raw_data.select(function(_token) {
      if (Object.prototype.toString.call(tokenOrTokens) == '[object Array]') {
        return tokenOrTokens.any(_token[0]);
      } else {
        return tokenOrTokens == _token[0];
      } 
    }).collect(function(_token) {
      return _token[1];
    });
  } 
  
  var find = function(token) {
    return findAll(token)[0];
  }

  // window.console.log('new SetBuilder.Trait()');
  // window.console.log(_raw_data);
  var _name = find('name');
  var _modifiers = findAll('modifier');
  var _direct_object_type = find('direct_object_type');
  ...
  this.prefix = function(negative) {
    var strings = findAll('string')
  }
  
  this.prefix = function(negative) {
    return findAllBefore('string', 'name');
  }
   
  this.toString = function(include_prefix, negative) {
    var tokens = ['string'];
    if (negative) tokens.push('negative');
    return findAll(tokens).join(' ') + ' ' + this.name();
  }
...
// .any
//
Array.prototype.__any = function(functionOrValue) {
  var results;
  if(typeof(functionOrValue) == "function") {
    results = this.__select(functionOrValue);
  } else {
    results = this.__select(function(value) {
      return value === functionOrValue;
    });
  }
  return results.length > 0;
}
if(!Array.prototype.any) Array.prototype.any = Array.prototype.__any;
