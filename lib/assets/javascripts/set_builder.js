...
SetBuilder.Set = function(_raw_data) {
  ...
  if(_raw_data.constructor === Array) {
  ...
  } else if (typeof _raw_data == 'object') {
    Object.keys(_raw_data).__each(function(key) {
      var traitHash = _raw_data[key];
      var trait_name = traitHash.trait;
      var negative = !!traitHash.negative;
      var trait = SetBuilder.traits().__find(trait_name);
      var args = [];
      if(traitHash.direct_object) args.push(traitHash.direct_object);
      
      if(traitHash.modifiers) {
        Object.keys(traitHash.modifiers).__each(function(key) {
          var modifierHash = traitHash.modifiers[key];
          var convertedModifier = {};
          convertedModifier[modifierHash.operator] = modifierHash.values ? Object.values(modifierHash.values) : [];
          args.push(convertedModifier);
        });
      }
